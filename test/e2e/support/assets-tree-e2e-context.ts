...
export type AssetsTreeE2eContext = {
  app: any;
  client: ClientProxy;
...
export async function setupAssetsTreeE2eContext(): Promise<AssetsTreeE2eContext> {
...
export async function teardownAssetsTreeE2eContext(
  context: AssetsTreeE2eContext,
): Promise<void> {
  await context.prisma.mntAssetsTree.deleteMany({});
...
export function defaultAssetsTreePayload(
  context: AssetsTreeE2eContext,
...
export async function createAssetsTreeRecord(
  context: AssetsTreeE2eContext,
  overrides: Record<string, unknown> = {},
): Promise<any> {
  return sendPattern(
    context.client,
    "assets.tree.create",
    defaultAssetsTreePayload(context, overrides),
  );
}

export async function getMockAssetsTreeIds(
  context: AssetsTreeE2eContext,
): Promise<string[]> {
  ...
  return response.assetsTree.map((r: any) => r.id);
}
